...
class MyMongoRealmClient {
    ...
    init(client: MongoClient?, app: App) {
        self.client = client
        self.app = app
    }
    ...
    func signInWithUsernamePassword(
        ...
        print("signInWithUsernamePassword (MyMongoRealmClient.swift)")
        self.app.login(
            credentials: Credentials.emailPassword(email: username, password: password)
        ) { authResult in
            switch authResult {
            case .success(let user):
                print("signInWithUsernamePassword in new realm style -> success (MyMongoRealmClient.swift)")
                onCompleted(user.toMap())
                break
    ...
    func logout(
        ...
        self.app.currentUser?.logOut(
            completion: { error in
                ...
                onCompleted(true)
                print("logout successfully (MyMongoRealmClient.swift)")
            }
        )
    }
...
//            }
    ...
    func callFunction(name: String,
        ...
        guard let user = self.app.currentUser else {
            fatalError("Logged out? (MyMongoRealmClient.swift)")
        }

        var argsBson = [AnyBSON]()
        args?.forEach { value in
            //argsBson.append(value as! AnyBSON)
            print("callFunction (MyMongoRealmClient.swift), args value: \(value)")
